...
## 2026-09-12 — iOS demo app (ANCDemo): four build stages complete, real on-device numbers verified; three teaching docs written; accidental model-artifact commit found and rewritten out of git history

**Scope of this entry:** back-filling the iOS/Xcode app build (never previously logged — built across an earlier session, evidenced only by in-code comments and the Xcode project itself, not by prior `logs.md` entries) plus this session's own two pieces of work: (1) three long-form teaching documents under `docs/`, and (2) discovery and remediation of a real repo-hygiene incident (model binaries committed and pushed to `origin/main` by mistake). No training, export, or quantization code was touched this session — v5 and all three `.mlpackage` precision variants remain exactly as verified in the prior entries above.

### iOS demo app — `ANCDemo/` (Xcode project, target: iPhone, model: `dns48_finetuned_v5_int8.mlpackage`)

Built as a native SwiftUI app (bundle ID `com.arnav.anc.ANCDemo`, automatic code signing, team ID `MGWJ67MC23`) across four incremental stages, each proving a specific capability before the next was attempted — evidenced directly in the real source files' own header comments (`ModelRunner.swift`, `ContentView.swift`, `AudioRecorder.swift`, `AudioPlayer.swift`, `ShareSheet.swift`), not reconstructed from memory:

- **Stage 1 — model load & verify.** Loaded the shipped INT8 `.mlpackage` on-device via Xcode's auto-generated Swift interface (`dns48_finetuned_v5_int8`), ran 4 back-to-back `prediction()` calls on the same model instance against a bundled test file (`test_gunshot_noisy.wav`), confirming the model loads and runs correctly on a physical device before any recording/UI code was attempted. This run is also where this project's real on-device latency numbers were first measured (see below). Model input contract independently verified against the export-side code (`src/export/traceable_demucs.py`, `src/export/to_coreml.py`, `src/export/verify_onnx.py`'s `_fit_to_fixed_length`), not assumed: 16kHz mono Float32, exactly 64,000 samples (4.0s), tensor names `noisy_waveform`/`enhanced_waveform`, shape `(1, 1, 64000)` — the traced/converted graph is valid ONLY at this exact length.
- **Stage 2 — record → infer → play.** `AudioRecorder` (`AVAudioRecorder`, deliberately not `AVAudioEngine` — this app needs "record to a file, then process," not live streaming, and real-time streaming inference is explicitly out of scope per `context.md`) records directly at 16kHz mono Float32, matching the model's input format exactly with no conversion step. Reuses the exact same shared model instance and `fitToFixedLength` fit-to-64000 logic Stage 1 verified (`ModelRunner.process(rawSamples:)`), not a second implementation. Introduced the **silent launch warmup** (`ModelRunner.warmup()`, fired from a background `Task.detached` in `ContentView`'s `.task {}`) and Raw/Enhanced A/B playback via `AudioPlayer`.
- **Stage 3 — Clean & Share / AirDrop.** Added a "Clean & Share" button presenting iOS's native `UIActivityViewController` (wrapped for SwiftUI in `ShareSheet.swift`) with the enhanced audio file — surfaces AirDrop (plus Messages/Mail/Save to Files) automatically, no AirDrop-specific code needed. Enhanced output is named `cleaned_speech_<timestamp>.wav` (not a raw UUID) so it's recognizable on the receiving device after sharing.
- **Stage 4 — UI polish.** Status-state UI (Ready/Recording/Processing/Done/Error, distinct colors), pulsing recording indicator, live recording-duration readout, on-screen inference-time readout, and playback-state management in `AudioPlayer` that always stops one source before starting the other (fixes a real found issue: two `AVAudioPlayer` instances mixing and sounding "confusing" when Raw/Enhanced could overlap).

**Real on-device latency numbers (Stage 1, 4 back-to-back calls, same model instance, same input):** **~274ms cold (first call), ~53–55ms warm (every call after)** — the same cold/warm-per-unique-shape pattern already found on the Mac's MPS backend in the Phase 3a follow-up (first-touch kernel dispatch/compilation cost, paid once per shape, not a one-time global cost), now confirmed to also hold on Core ML/iPhone. **Design decision — silent launch warmup:** since the app's input length is fixed at exactly 64,000 samples, `ModelRunner.warmup()` runs one throwaway prediction on the bundled test file at app launch (invisibly, no UI tied to it, fire-and-forget from a background task) specifically so the cold ~274ms cost is absorbed before the user ever taps record — the first real recording's displayed inference time reflects the warm ~53–55ms number, not the cold one.

**Xcode/device pairing chain (real debugging cost, recorded for reuse on future devices):** getting a locally-built app running on a physical iPhone required four independent, real links in a trust chain — Developer Mode enabled on-device, valid code signing (this project uses `CODE_SIGN_STYLE = Automatic` against team ID `MGWJ67MC23`), Keychain access to the signing certificate, and explicit on-device developer-trust of the certificate (Settings → General → VPN & Device Management) before first launch — each capable of failing independently with a distinct, sometimes cryptic Xcode error. Not previously written down anywhere in this repo; captured now in `docs/ios_app_brief.md` so it doesn't need to be re-derived from scratch on the next device pairing.

**Status: all four stages complete and functionally verified on a physical device** (model load, record→infer→play, share/AirDrop, and UI polish all confirmed working per the above). No formal SNR/STOI/PESQ re-verification was run against on-device inference output specifically (the shipped INT8 `.mlpackage`'s accuracy numbers are the ones already verified on-Mac in the Phase 5b INT8 entry above, via `coremltools`' `predict()` API, not via the iOS app itself) — the app's own verification scope is model-loads-and-runs-correctly-on-device plus the real latency numbers, not a second independent accuracy re-check.

### Three teaching documents written — `docs/`

Per explicit request, wrote three long-form, beginner-to-master-level documents grounded entirely in this project's real files, config values, and logged numbers (`context.md`, `logs.md`, `configs/finetune.yaml`, `src/model/finetune.py`, `src/vendor/denoiser_patched/stft_loss.py`, and the real ANCDemo Swift source/comments) — no invented or approximated numbers, per explicit instruction. Writing-only task; no code was run.

- `docs/phase3_training_deep_dive.md` (~7,325 words) — transfer learning/fine-tuning fundamentals, the training loop mechanics with real `finetune.py` code, the L1 + multi-resolution STFT loss derived in full (including why L1 over L2 for audio, the real `fft_sizes=[1024,2048,512]`/`hop_sizes`/`win_lengths` multi-resolution config), warmup+cosine vs. `ReduceLROnPlateau` LR scheduling derived with real v2/v5 config values, the full v1→v5 diagnostic journey (over-suppression diagnosis, data-gap cross-tabulation, static-dataset-exhaustion root cause, v5's dynamic-mixing fix) as the centerpiece, the Δ-SNR-vs-absolute-SNR>15dB reporting rationale worked through numerically, and the final v5 results with honest PS-target assessment.
- `docs/export_quantization_deep_dive.md` (~5,220 words) — ONNX as a bridge format from first principles, the real `torch.jit.trace`/`TraceableDemucs` bug (the Python-int-arithmetic-traced-as-tensor-ops hazard, the `TracerWarning`, the `coremltools` 0-d-array crash, the precomputed-constants fix, and the real 0.0-diff verification), the full PyTorch→ONNX→CoreML-fp32→fp16→int8 verification chain with every real delta number and the isolation methodology behind it, fp32/fp16/int8 explained from first principles (bit layout, the quantization/dequantization formula, why LSTMs are historically riskier to quantize than conv layers plus this project's real per-SNR-bucket check for that failure mode), and the fp32-vs-fp16/fp32-vs-int8 decisions as real "measure then decide" engineering tradeoffs.
- `docs/ios_app_brief.md` (~1,286 words, intentionally brief) — Core ML/`.mlpackage` basics, the Xcode/device pairing chain, the four build stages, and the real cold/warm latency numbers plus the silent-warmup design decision.

Word counts and section lists were reported back and reviewed before this log entry was written, per explicit instruction not to append until the docs were sanity-checked.

### Repo-hygiene incident found and fixed: model binaries committed and pushed by mistake, git history rewritten

**Trigger:** user noticed a prior commit (`a54e699`, message "prototype over") had pushed ONNX/Core ML model artifacts to `origin/main` by mistake, and asked for `.gitignore` to be checked.

**Root cause, confirmed by direct inspection (not assumed):** `.gitignore` excluded `*.onnx` and `*.pt`, but had no rule for `*.onnx.data` (ONNX's external-weights sidecar file, produced by the Phase 5a export) or `*.mlpackage` (Core ML's bundle format — git sees this as a directory of files, which `*.onnx`/`*.pt` patterns never matched). As a result, `git ls-files` confirmed the following were tracked and already pushed: all three Phase 5b `.mlpackage` variants (`checkpoints/coreml/dns48_finetuned_v5{,_fp16,_int8}.mlpackage`), both Phase 5a `.onnx.data` sidecars (`checkpoints/onnx/{dns48_finetuned_v5,dynamic_dns48_finetuned_v5}.onnx.data`), and a duplicate copy of the int8 `.mlpackage` bundled into `ANCDemo/ANCDemo/Resources/` for the iOS app — roughly 300MB total, all reachable from `origin/main`, confirmed via `git ls-remote`/`git log origin/main` before any fix was attempted.

**Fix, per user's explicit choice of full history rewrite over a simpler forward-only fix:**
1. Backed up the full repo (`git clone --mirror`) to the session scratchpad before any destructive operation, per standing safety practice for irreversible git operations.
2. Confirmed working tree was fully clean (`git status` — nothing uncommitted) before proceeding, so no in-progress work was at risk.
3. Installed `git-filter-repo` (`brew install git-filter-repo` — not previously available) and used it to strip `checkpoints/coreml`, `checkpoints/onnx`, and `ANCDemo/ANCDemo/Resources` from **every commit in history** (`--invert-paths`), not just the latest one — rewrote all 5 commit hashes as an unavoidable consequence of a history-content rewrite.
4. Updated `.gitignore`: added `*.onnx.data`, `*.mlpackage/`, `checkpoints/coreml/`, `checkpoints/onnx/`, `ANCDemo/ANCDemo/Resources/*.mlpackage/` — closes the actual gap that let this happen, not just a one-off cleanup.
5. Force-pushed the rewritten history to `origin/main` (`git push --force`) — done only after explicit, scoped user confirmation of exactly this operation, per this project's git-safety standard for destructive/shared-history operations.

**Verification performed after the rewrite (not assumed clean from "the command didn't error"):** `git ls-files` (local) and `git log origin/main -- <the three stripped paths>` (remote, after `git fetch`) both confirmed zero references to the stripped paths anywhere in history, on both local and remote. `.git` directory size dropped from 300MB+ to 656KB. All 5 original commits' content-equivalent history is preserved (same commit messages, same file changes minus the three stripped paths), just under new hashes. Model files themselves were NOT deleted from local disk — only untracked from git going forward; `checkpoints/coreml/`, `checkpoints/onnx/`, and the app's bundled `.mlpackage` remain present and usable locally.

**Flagged to user, not silently assumed:** any other local clone or fork of this repo (another machine, a collaborator) is now diverged from the rewritten history and will need a fresh clone, not a pull, to reconcile — force-pushing rewritten history breaks `git pull` for anyone else already on the old `a54e699`-based history.

**Not done this session:** no training, export, or quantization code was touched or re-run; v5's checkpoint and all three verified `.mlpackage` precision variants are unchanged from the prior entries above. `requirements.txt`/`onnx`/`onnxruntime`/`coremltools` pinning (flagged as open in earlier entries) remains open, not addressed here.
